...
import { isFilePublished, sortByDate } from "./common";
...
export function getFilenames(type, filter = () => true) {
  return fs.readdirSync(getDirectory(type)).filter(filter);
}
...
export function getPublished(type, fields = [], limit) {
  const filenames = getFilenames(type, isFilePublished);
  const limitItems = limit || filenames.length;
  const items = filenames
    .map((filename) => getByFilename(type, filename, fields))
    // sort posts by date in descending order
